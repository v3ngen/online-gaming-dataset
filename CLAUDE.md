...
2. **Spec**: document the target features, their real-world/research grounding, feature relationships/correlations to build in, and the intentional data quality issues (missing values, noise/anomalies, duplicates) — students must be able to discover these deliberately, so they need to be spelled out precisely (rates, affected columns, detection/cleaning approach).
3. **Generation scripts**: Python (`generate_dataset.py`, plus a `validate_dataset.py` that checks ranges, distributions, correlations and quality-issue counts against the spec), seeded for reproducibility.
